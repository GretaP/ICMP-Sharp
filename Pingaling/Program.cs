		...
		static void Main(string[] args) {
			...
			try {
				...
				for (int i = 0; i < args.Length; i++) {
					...
					case "--count":
					case "-c":
						if (args.Length <= i || !int.TryParse(args[i + 1], out count)) throw new ArgumentException();
						i++;
						continue;
					case "-t":
						if (args.Length <= i || !int.TryParse(args[i + 1], out timeout)) throw new ArgumentException();
						i++;
						continue;
					}
					url = args[i];
					...
					for (int i = 0; i < count; i++) {
                       ...
                       */
						...
						case IPStatus.Success:
							if (!string.IsNullOrEmpty(realhost) && url != realhost) Console.Write($ "Reply from {realhost};");
							else Console.Write($ "Reply from");
							...
							if (reply.RoundtripTime < mintime || mintime == 0) mintime = reply.RoundtripTime;
